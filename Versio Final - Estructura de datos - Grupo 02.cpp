#include <iostream>
#include <string>
#include <fstream>   // Librería para la persistencia de archivos (lectura/escritura)
#include <ctime>     // Librería para CTIMER y medir el ciclo de vida del proceso
#include <windows.h> // Librería para simular el consumo de tiempo con Sleep()
...
public:
    ...
    // --- MÓDULO DE PERSISTENCIA Y MANEJO DE ARCHIVOS ---
    
    // Método para guardar el estado actual de la lista en un archivo físico (.txt)
    void guardarEnArchivo() {
        ofstream archivo("procesos.txt"); // Crea o sobreescribe el archivo físico
        NodoLista* aux = cabeza;
        while (aux != NULL) {
    ...
    // Método para cargar procesos desde un archivo físico al iniciar el sistema
    void cargarDeArchivo() {
        ...
        Proceso p;
        // Lee línea por línea e inserta en la lista enlazada
        while (archivo >> p.id >> p.nombre >> p.prioridad >> p.rafaga) {
            agregarProceso(p);
        }
        archivo.close();
        cout << "[Persistencia] Estado de procesos anteriores cargado con éxito.\n";
    }
};
...
public:
    ...
    void ejecutarProcesos(GestorMemoria& memoria) {
        ...
        while (frente != NULL) {
            ...
            // --- USO DE CTIMER PARA GESTIÓN DE TIEMPO REAL ---
            cout << "      Ejecutando rafaga solicitada (" << atendido->p.rafaga << "ms)...\n";
            clock_t tiempo_inicio = clock(); // Marca de tiempo al iniciar la ráfaga
            
            Sleep(atendido->p.rafaga); // Pausa el sistema físicamente simulando que el CPU está trabajando
            
            clock_t tiempo_fin = clock(); // Marca de tiempo al finalizar la ráfaga
            
            // Cálculo del tiempo real consumido mediante la librería CTIMER
...
int main() {
    ...
    // Al arrancar el sistema, cargamos automáticamente la información guardada en disco
